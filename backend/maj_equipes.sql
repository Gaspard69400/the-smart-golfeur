-- ════════════════════════════════════════════
-- THE SMART GOLFER — Formats par équipe dans la carte partagée (S62)
-- Scramble, greensome, foursome.
-- À coller EN ENTIER dans : Supabase → SQL Editor → New query → Run
-- Idempotent (on peut le relancer sans risque).
-- Nécessite maj_parties_carnet.sql déjà passé.
--
-- Une équipe = 1 ou 2 (colonne `team`). Le score de l'équipe est noté sur
-- la ligne de son premier joueur : set_game_score ne change pas.
-- ════════════════════════════════════════════

-- 1) Nouveaux formats + équipe du joueur
alter table public.shared_games drop constraint if exists shared_games_format_check;
alter table public.shared_games add constraint shared_games_format_check
  check (format in ('stroke', 'stableford', 'match', 'scramble', 'greensome', 'foursome'));

alter table public.shared_game_players add column if not exists team smallint;
alter table public.shared_game_players drop constraint if exists shared_game_players_team_check;
alter table public.shared_game_players add constraint shared_game_players_team_check check (team is null or team in (1, 2));

-- 2) Créer une partie : p_players accepte "team": 1|2 (obligatoire pour les formats par équipe)
...
declare
  ...
  v_team_fmt boolean; v_t1 int; v_t2 int; v_bad int;
begin
  if v_uid is null then return jsonb_build_object('ok', false, 'error', 'Non connecté'); end if;
  if p_format not in ('stroke', 'stableford', 'match', 'scramble', 'greensome', 'foursome') then
    return jsonb_build_object('ok', false, 'error', 'Format inconnu');
  end if;
  ...
  v_team_fmt := p_format in ('scramble', 'greensome', 'foursome');
  if v_team_fmt then
    select count(*) filter (where value ->> 'team' = '1'),
           count(*) filter (where value ->> 'team' = '2'),
           count(*) filter (where coalesce(value ->> 'team', '') not in ('1', '2'))
      into v_t1, v_t2, v_bad
      from jsonb_array_elements(p_players);
    if v_bad > 0 then return jsonb_build_object('ok', false, 'error', 'Chaque joueur doit être dans une équipe'); end if;
    if p_format = 'scramble' then
      if (v_t1 > 0 and v_t1 < 2) or (v_t2 > 0 and v_t2 < 2) then
        return jsonb_build_object('ok', false, 'error', 'Au moins 2 joueurs par équipe');
      end if;
    elsif (v_t1 not in (0, 2)) or (v_t2 not in (0, 2)) then
      return jsonb_build_object('ok', false, 'error', 'Ce format se joue par équipes de 2');
    end if;
    if v_t1 + v_t2 < 2 then return jsonb_build_object('ok', false, 'error', 'Au moins une équipe de 2'); end if;
  end if;
  ...
  for v_pl in select value from jsonb_array_elements(p_players) loop
    ...
    insert into public.shared_game_players (game_id, user_id, name, initials, color, hcp, position, team)
    values (v_id, v_user, left(coalesce(nullif(trim(v_pl ->> 'name'), ''), 'Joueur'), 40),
            ...
            case when v_team_fmt then (v_pl ->> 'team')::smallint else null end)
...
-- 3) Aperçu : l'équipe de chaque place
...
begin
  ...
  select coalesce(jsonb_agg(jsonb_build_object('id', p.id, 'name', p.name, 'hcp', p.hcp, 'team', p.team,
...
-- 4) Rejoindre : en équipe, on prend une place existante (les équipes sont fixées au départ)
...
begin
  ...
  if g.format in ('scramble', 'greensome', 'foursome') then
    return jsonb_build_object('ok', false, 'error', 'Partie en équipe : choisis ta place dans la liste');
  end if;
...
-- 5) Invité en cours de partie : pas en équipe (les équipes sont fixées au départ)
...
begin
  ...
  if g.status <> 'live' then return jsonb_build_object('ok', false, 'error', 'La partie est terminée'); end if;
  if g.format in ('scramble', 'greensome', 'foursome') then return jsonb_build_object('ok', false, 'error', 'Les équipes sont fixées au départ'); end if;
...
-- 6) Droits (inchangés, rappelés par sécurité)
